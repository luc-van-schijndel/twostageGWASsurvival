...
#Associated read function to be passed to the batch-function: function(x) as.matrix(read.table(x))
```

The batch function allows for the last file to have less covariates than the other files. This is however the only flexibility implemented; the function assumes all other files are structured similarly, e.g. same dimensions, same presence of headers, etc.
