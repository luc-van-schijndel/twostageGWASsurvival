...
test_that("First stage function gives properly structured output", {
  ...
  expect_length(firststagecoxph(survival.dataset, covariate.matrix, progress = 0), 3)
  expect_lt(firststagecoxph(survival.dataset, covariate.matrix, progress = 0)[1], 1)
  expect_gt(firststagecoxph(survival.dataset, covariate.matrix, progress = 0)[1], 0)
})
